@extends ('layouts.app', ['title' => 'Tag - Index'])
                        ...
                        <div class="flex flex-1 flex-wrap">
                            ...
                            <div class="w-full p-3">
                                ...
                                <div class="max-w-full lg:max-w-3xl xl:max-w-5xl">
                                    ...
                                    <div class="p-3">
                                        <div class="p-4">
                                            <h5 class="font-bold text-gray-400"> <i class="	fas fa-film fa-fw mr-3"></i>Article / Tag</h5>
                                        </div>
                                        
                                        <div class="flex">
                                            @can('tags.create')
                                            <button data-modal-toggle="create-modal" class="text-white focus:outline-none bg-gray-600 hover:bg-gray-500 px-2 py-1 shadow-sm rounded-md">
                                                <i class="fas fa-file-alt mr-2"></i> Add New Tag
                                            </button>
                                            @endcan
                        ...
                        <div class="flex flex-row flex-wrap flex-grow mt-2">              
                            ...
                            <div class="bg-white border rounded shadow">
                                <div class="border-b p-3">
                                    <h5 class="font-bold text-gray-600 text-sm">Table Tags</h5>
                                </div>
                                <div class="p-5">
                                    <table class="w-full p-5 text-gray-600 text-sm">
                                        <thead class="bg-gray-200">
                                            <tr>
                                                ...
                                                <th class="text-left text-gray-600 text-sm p-1">Slug</th>
                                                <th class="text-left text-gray-600 text-sm p-1">Date Created</th>                                       
                                        ...
                                        <tr class="border bg-white">
                                            <th scope="row" class=" text-left p-1">{{ ++$no + ($tags->currentPage()-1) * $tags->perPage() }}</th>
                                            <td class=" text-left p-1">{{ $tag->name }}</td>
                                            <td class=" text-left p-1">{{ $tag->slug }}</td>
                                            <td class=" text-left p-1">{{ $tag->created_at }}</td>
                                            <td class="px-5 py-1 text-center">
                                                @can('tags.edit')
                                                <a href="{{ route('admin.tag.edit', $tag->id) }}" class="bg-gray-500 px-2 py-0.5 rounded shadow-sm text-xs text-white focus:outline-none"> 
                                                <i class="fas fa-edit text-center"></i></a>
                                                @endcan
                                                @can('tags.delete')
                                                <button onClick="destroy(this.id)" id="{{ $tag->id }}" class="bg-yellow-500 px-2 py-1 rounded shadow-sm text-xs text-white focus:outline-none fas fa-trash-alt"></button>
                                                @endcan
                                            </td>
                                        </tr>
                                        @empty
