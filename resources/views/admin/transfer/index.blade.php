@extends ('layouts.app', ['title' => 'ticash | Transfer'])
            ...
            <div class="flex flex-1 flex-wrap">
                ...
                <div class="w-full p-3">
                    ...
                    <div class="max-w-full lg:max-w-3xl xl:max-w-5xl">
                        ...
                        <div class="p-3">
                            <div class="p-4">
                            <h5 class="font-bold text-gray-400"> <i class="	fas fa-server fa-fw mr-3"></i>tiSharing</h5>
                            </div>
                    ...
                    <div class="bg-white border rounded shadow">
                        ...
                        <div class="p-5">
                            <table class="w-full p-5 text-gray-600 text-sm">
                                ...
                                @forelse($transfers as $no => $transfer)
                                <tr class="border bg-white">
                                    <th scope="row" class=" text-left p-1">{{ ++$no + ($transfers->currentPage()-1) * $transfers->perPage() }}</th>
                                   
                                    <td class=" text-left p-1">{{ $transfer->trans_number }}</td>
                                    <td class=" text-left p-1">{{ $transfer->institution->institution_name }}</td>
                                    <td class=" text-left p-1">{{ $transfer->user->name }}</td>
                                    <td class=" text-left p-1">{{ moneyFormat($transfer->amount) }}</td>
                                    <td class=" text-left p-1">{{ $transfer->transaction_category->transaction_name }}</td>
                                    <td class=" text-left p-1">{{ $transfer->description }}</td>
                                    <td class=" text-left p-1">{{ $transfer->status }}</td>
        ...
        <div class="flex flex-col w-11/12 sm:w-5/6 lg:w-1/2 max-w-2xl mx-auto rounded-lg border border-gray-300 shadow-xl">
            ...
            <div class="bg-white rounded-lg shadow relative dark:bg-gray-700">
                <div class="flex justify-end p-2 border-b">
                    <h3 class="text-xl font-medium text-gray-900 dark:text-white">Add transfer Form</h3>
                ...
                <form class="space-y-6 px-6 lg:px-8 pb-4 sm:pb-6 xl:pb-8" action="{{ route('admin.transfer.store')}}" method="POST" enctype="multipart/form-data">
                @csrf
                    <div class="grid gap-1 grid-cols-1 grid-rows-1">
                        <div class="mr-2">
                            <label for="s" class="text-sm font-medium text-gray-900 block mb-2 dark:text-gray-300">Phone Number</label>
                            <input type="number" name="phone_number" value="{{ old('phone_number') }}" placeholder="Search" class="w-full bg-white-700 text-sm text-gray-800 transition border focus:outline-none focus:border-gray-700 rounded py-1 px-7 pl-20 appearance-none leading-normal">
