@extends ('layouts.app', ['title' => 'ticash | Topup'])
            ...
            <div class="flex flex-1 flex-wrap">
                ...
                <div class="w-full p-3">
                    ...
                    <div class="max-w-full lg:max-w-3xl xl:max-w-5xl">
                        ...
                        <div class="p-3">
                            <div class="p-4">
                                <h5 class="font-bold text-gray-400"> <i class="	fas fa-server fa-fw mr-3"></i>Topup</h5>
                            </div>
                    ...
                    <div class="bg-white border rounded-3xl shadow">
                        ...
                        <div class="p-5">
                            <table class="w-full p-5 text-gray-600 text-sm">
                                ...
                                @forelse($topups as $no => $topup)
                                <tr class="border bg-white">
                                    <th scope="row" class=" text-left p-1">{{ ++$no + ($topups->currentPage()-1) * $topups->perPage() }}</th>
                                   
                                    <td class=" text-left p-1">{{ $topup->trans_number }}</td>
                                    <td class=" text-left p-1">{{ $topup->institution->institution_name }}</td>
                                    <td class=" text-left p-1">{{ $topup->user->name }}</td>
                                    <td class=" text-left p-1">{{ moneyFormat($topup->amount) }}</td>
                                    
                                    <td class=" text-left p-1">{{ $topup->description }}</td>
                                    <td class=" text-left p-1">{{ $topup->status }}</td>
                                    ...
                                    <label for="amount" class="text-sm font-medium text-gray-900 block mb-2 dark:text-gray-300">-</label>                            
                                        ...
                                        @elseif($topup->status == 'processing')
                                      
                                        @can('transactions.edit')
                                        <a href="{{ route('admin.topup.edit', $topup->id) }}" class="bg-gray-600 px-2 py-0.5 rounded shadow-sm text-xs text-white focus:outline-none">
                                        <i class="fas fa-edit text-center"></i></a>
                                        @endcan        
                                        @can('transactions.delete')
                                        <button onClick="destroy(this.id)" id="{{ $topup->id }}" class="bg-yellow-600 px-2 py-1 rounded shadow-sm text-xs text-white focus:outline-none fas fa-check"></button>
                                        @endcan
                                        @else
                                        @can('transactions.edit')
                                        <a href="{{ route('admin.topup.edit', $topup->id) }}" class="bg-gray-600 px-2 py-0.5 rounded shadow-sm text-xs text-white focus:outline-none">
    ...
    <div id="create-modal" aria-hidden="true" class="hidden overflow-x-hidden overflow-y-auto fixed h-modal md:h-full top-4 left-0 right-0 md:inset-0 z-50 justify-center items-center">
        <div class="flex flex-col w-11/12 sm:w-5/6 lg:w-1/2 max-w-2xl mx-auto rounded-3xl border border-gray-300 shadow-xl">
            ...
            <div class="bg-white rounded-3xl shadow relative dark:bg-gray-700">
                ...
                <form class="space-y-6 px-6 lg:px-8 pb-4 sm:pb-6 xl:pb-8" action="{{ route('admin.topup.store')}}" method="POST" enctype="multipart/form-data">
                @csrf
