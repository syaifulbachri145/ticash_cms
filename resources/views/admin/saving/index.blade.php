@extends ('layouts.app', ['title' => 'ticash | Saving'])
                ...
                <div class="w-full p-3">
                    ...
                    <div class="max-w-full lg:max-w-3xl xl:max-w-5xl">
                        ...
                        <div class="p-4">
                                ...
                                tiSaving:  <strong>{{moneyFormat($savingBalance)}} </strong>
                                </h5>
                            ...
                            <div class="flex">
                                ...
                                @endcan
                                <div class="relative pull-right pl-4 pr-10 md:pr-20">
                                    <form action="{{ route('admin.saving.index') }}" method="GET">
                                    <input type="search" name="q" value="{{ request()->query('q') }}" placeholder="Search Title/User/Class" class="w-full bg-white-700 text-sm text-gray-800 transition border focus:outline-none focus:border-gray-700 rounded-2xl py-1 px-7 pl-10 appearance-none leading-normal">
                                    </form>
                        ...
                        <div class="p-5">
                            <table class="w-full p-5 text-gray-600 text-sm">
                                <thead class="bg-gray-200">
                                   ...
                                   <tr>
                                        <th colspan="2">
                                            <a class="btn btn-warning float-end" href="{{ route('admin.savingReport.index') }}"><i class="fa fa-download"></i> Export Data</a>
                                        </th>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <th class="text-left text-gray-600 text-sm p-1">Last Saving</th>
                                        <th class="text-center text-gray-600 text-sm p-1">Action</th>
                                    </tr>
                                </thead>
                                <tbody class="bg-gray-200">
                                @forelse($savings as $no => $saving)
                                <tr class="border bg-white">
                                    <th scope="row" class=" text-left p-1">{{ ++$no + ($savings->currentPage()-1) * $savings->perPage() }}</th>
                                    <td class=" text-left p-1">{{ $saving->description }}</td>
                                    <td class=" text-left p-1">{{ $saving->user_name }}</td>
                                    @if($saving->user->access_id == '5')
                                    <td class=" text-left p-1">{{ $saving->degree_name }}</td>
                                    @else
                                    <td class=" text-left p-1">-</td>
                                    @endif
                                    <td class=" text-left p-1">{{ $saving->goal }}</td>
                                    <td class=" text-left p-1">{{ moneyFormat($saving->balance) }}</td>
                                    <td class=" text-left p-1">{{ $saving->deadline }}</td>
                                    <td class=" text-left p-1">{{ $saving->status }}</td>
                                    <td class=" text-left p-1">{{ $saving->updated_at }}</td>
                                    <td class="px-5 py-1 text-center">
                                  
                                        @can('savings.edit')
                                        <a href="{{ route('admin.saving.edit', $saving->id) }}" class="bg-gray-500 px-2 py-0.5 rounded shadow-sm text-xs text-white focus:outline-none">
                                        <i class="fas fa-edit text-center"></i></a>
                                        @endcan

                                      @if($access == '1')
                                          @can('savings.delete')
                                        <button onClick="destroy(this.id)" id="{{ $saving->id }}" class="bg-yellow-500 px-2 py-1 rounded shadow-sm text-xs text-white focus:outline-none fas fa-check"></button>
                                        @endcan
                                      @endif  
        ...
        <div class="flex flex-col w-11/12 sm:w-5/6 lg:w-1/2 max-w-2xl mx-auto rounded-lg border border-gray-300 shadow-xl">
            ...
            <div class="bg-white rounded-lg shadow relative dark:bg-gray-700">
                <div class="flex justify-end p-2 border-b">
                    <h3 class="text-xl font-medium text-gray-900 dark:text-white">Add saving Form</h3>
                    ...
                    success: function (response) {
                        if (response.status == "success") {
                            Swal.fire({
                                icon: 'success',
                                title: 'Success!',
                                text: 'Your Saving has been collected.!',
