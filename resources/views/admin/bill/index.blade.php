                    ...
                    <div class="max-w-full lg:max-w-3xl xl:max-w-5xl">
                        ...
                        <div class="p-3">
                            ...
                            <h5 class="text-gray-500"> 
                                Invoice<strong> {{moneyFormat($institution->invoice)}}</strong>
                            </h5>
                            </div>
                    ...
                    <div class="p-5">
                            <table class="w-full p-5 text-gray-600 text-sm">
                                <thead class="bg-gray-200">
                                    <tr>
                                        <th colspan="2">
                                            <a class="btn btn-warning float-end" href="{{ route('admin.billReport.index') }}"><i class="fa fa-download"></i> Export Data</a>
                                        </th>
                                    </tr>
                                    <tr>
                                        ...
                                        <th class="text-left text-gray-600 text-sm p-1">Bank Transfer</th>
                                        <th class="text-left text-gray-600 text-sm p-1">Account Name</th>
                                ...
                                @forelse($bills as $no => $bill)
                                <tr class="border bg-white">
                                    <th scope="row" class=" text-left p-1">{{ ++$no + ($bills->currentPage()-1) * $bills->perPage() }}</th>
                                    <td class=" text-left p-1">{{ $bill->institution->institution_name }}</td>
                                    <td class=" text-left p-1">{{ $bill->user->name }}</td>
                                    <td class=" text-left p-1">{{ moneyFormat($bill->amount) }}</td>
                                    <td class=" text-left p-1">{{ $bill->bank_transfer }}</td>
                                    <td class=" text-left p-1">{{ $bill->account_name }}</td>
                                    <td class=" text-left p-1">{{ $bill->description }}</td>
                                    <td class=" text-left p-1">{{ $bill->status }}</td>
                                    <td class="px-5 py-1 text-center">
                                  
                                    @if($bill->status == 'success')
                                    <label for="amount" class="text-sm font-medium text-gray-900 block mb-2 dark:text-gray-300">-</label>                            
                                        @else

                                        @if($bill->institution_id == Auth()->user()->institution_id)
                                        <label for="amount" class="text-sm font-medium text-gray-900 block mb-2 dark:text-gray-300">-</label>           

                                        @else

                                        @can('bills.edit')
                                        <a href="{{ route('admin.bill.edit', $bill->id) }}" class="bg-gray-600 px-2 py-0.5 rounded shadow-sm text-xs text-white focus:outline-none">
                                        ...
                                        @can('bills.delete')
                                        <button onClick="destroy(this.id)" id="{{ $bill->id }}" class="bg-yellow-600 px-2 py-1 rounded shadow-sm text-xs text-white focus:outline-none fas fa-check"></button>
                                        @endif
                                        @endcan
                                    @endif
                                    </td>
        ...
        <div class="flex flex-col w-11/12 sm:w-5/6 lg:w-1/2 max-w-2xl mx-auto rounded-3xl border border-gray-300 shadow-xl">
            ...
            <div class="bg-white rounded-3xl shadow relative dark:bg-gray-700">
                <div class="flex justify-end p-2 border-b">
                    <h3 class="text-xl font-medium text-gray-900 dark:text-white">Add Bill Form</h3>
                ...
                <form class="space-y-6 px-6 lg:px-8 pb-4 sm:pb-6 xl:pb-8" action="{{ route('admin.bill.store')}}" method="POST" enctype="multipart/form-data">
                @csrf
                                ...
                                <div class="grid gap-1 grid-cols-2 grid-rows-1">
                                    ...
                                    <div class="mr-2">
                                        <label for="balance" class="text-md font-large text-gray-700 block mb-2 dark:text-gray-300">Bill: <br> <strong>{{moneyFormat($institution->invoice)}}</strong></label>  
                                    </div>
                                </div>
                    ...
                    <div class="grid gap-1 grid-cols-2 grid-rows-1">
                        ...
                        <div class="mr-2">
                            ...
                            <input type="text" name="bank_transfer" value="{{ old('bank_transfer') }}" id="bank_transfer" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-1.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="Bank Transfer" required="">
                            @error('bank_transfer')
                        ...
                        <div class="mr-2">
                            ...
                            <input type="text" name="account_name" value="{{ old('account_name') }}" id="account_name" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-1.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="Account Name" required="">
                            @error('account_name')
                                <div class="w-full bg-red-200 shadow-sm rounded-md overflow-hidden mt-2">
                                    <div class="px-4 py-2">
                                        <p class="text-gray-600 text-sm">{{ $message }}</p>
                    ...
                    <div class="grid gap-1 grid-cols-1 grid-rows-1">
                        ...
                        <div class="mr-2">
                            ...
                            <input type="hidden" name="amount" value="{{ old('amount',$institution->invoice) }}" id="amount" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-1.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="Amount" required="">
