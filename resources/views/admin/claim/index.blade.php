                    ...
                    <div class="max-w-full lg:max-w-3xl xl:max-w-5xl">
                        ...
                        <div class="p-3">
                            ...
                            <h5 class="text-gray-500"> 
                                Balance:<strong> {{moneyFormat(Auth()->user()->institution->balance)}}</strong>
                                </h5>
                            </div>
                            ...
                            <div class="flex">
                                ...
                                <div class="relative pull-right pl-4 pr-8 md:pr-20">
                                    <form action="{{ route('admin.topup.index') }}" method="GET">
                ...
                <div class="w-full p-3">
                    ...
                    <div class="p-5">
                            <table class="w-full p-5 text-gray-600 text-sm">
                                ...
                                <tr>
                                        <th colspan="2">
                                            <a class="btn btn-warning float-end" href="{{ route('admin.claimReport.index') }}"><i class="fa fa-download"></i> Export Data</a>
                                        </th>
                                    </tr>
                                    <tr>
                                        ...
                                        <th class="text-left text-gray-600 text-sm p-1">Request</th>
                                        <th class="text-left text-gray-600 text-sm p-1">Approval</th>
                                        <th class="text-left text-gray-600 text-sm p-1">Reqest Date</th>
                                        <th class="text-left text-gray-600 text-sm p-1">Approval Date</th>
                                        <th class="text-left text-gray-600 text-sm p-1">Status</th>
                                        <th class="text-center text-gray-600 text-sm p-1">Action</th>
                                    </tr>
                                </thead>
                                <tbody class="bg-gray-200">
                                @forelse($claims as $no => $claim)
                                <tr class="border bg-white">
                                    <th scope="row" class=" text-left p-1">{{ ++$no + ($claims->currentPage()-1) * $claims->perPage() }}</th>
                                    <td class=" text-left p-1">{{ $claim->institution->institution_name }}</td>
                                    <td class=" text-left p-1">{{ moneyFormat($claim->amount) }}</td>
                                    <td class=" text-left p-1">{{ $claim->description }}</td>
                                    <td class=" text-left p-1">{{ $claim->user->name }}</td>
                                    <td class=" text-left p-1">{{ $claim->user_approved }}</td>
                                    <td class=" text-left p-1">{{ $claim->request_date }}</td>
                                    <td class=" text-left p-1">{{ $claim->approval_date }}</td>
                                    <td class=" text-left p-1">{{ $claim->status }}</td>
                                    ...
                                    <label for="amount" class="text-sm font-medium text-gray-900 block mb-2 dark:text-gray-300">-</label>                            
                                        @else
                                        @can('claims.edit')
                                        <a href="{{ route('admin.claim.edit', $claim->id) }}" class="bg-gray-600 px-2 py-0.5 rounded shadow-sm text-xs text-white focus:outline-none">
                                        <i class="fas fa-edit text-center"></i></a>
                                        @endcan        
                                        @can('claims.delete')
                                        <button onClick="destroy(this.id)" id="{{ $claim->id }}" class="bg-yellow-600 px-2 py-1 rounded shadow-sm text-xs text-white focus:outline-none fas fa-check"></button>
                                        @endcan
                                    @endif
                                    </td>
                            ...
                            @if ($claims->hasPages())
                                <div class="bg-white p-3">
                                    {{ $claims->links('pagination::tailwind') }}
                                </div>
                            @endif
        ...
        <div class="flex flex-col w-11/12 sm:w-5/6 lg:w-1/2 max-w-2xl mx-auto rounded-lg border border-gray-300 shadow-xl">
            ...
            <div class="bg-white rounded-lg shadow relative dark:bg-gray-700">
                <div class="flex justify-end p-2 border-b">
                    <h3 class="text-xl font-medium text-gray-900 dark:text-white">Add topup Form</h3>
                ...
                <form class="space-y-6 px-6 lg:px-8 pb-4 sm:pb-6 xl:pb-8" action="{{ route('admin.claim.store')}}" method="POST" enctype="multipart/form-data">
                @csrf
                    ...
                    <div class="grid gap-1 grid-cols-1 grid-rows-1">
                        ...
                        <div class="mr-2">
                            ...
                            <input type="number" name="amount" value="{{ old('amount') }}" id="amount" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-1.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="Amount" required="">
                            @error('amount')
