@extends ('layouts.app', ['title' => 'Shop - Create'])
...
@section('content')
        ...
        <div id="main-content" class="w-full flex-1">
            ...
            <div class="flex flex-col w-full sm:w-5/6 lg:w-full max-w-3xl mx-auto rounded-lg shadow-xl mt-6">

                <div class="w-full p-3">
                    <!--Table Card-->
                    <div class="bg-white border rounded shadow">
                    <div class="border-b p-3">
                        <h3 class="text-xl font-medium text-gray-900 dark:text-white">Shop</h3>
                    </div>
                  
                        <form class="space-y-6 px-6 lg:px-8 pb-4 sm:pb-6 xl:pb-8" action="{{ route('admin.shop.update', $user->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="grid gap-1 grid-cols-3 grid-rows-1">
                                ...
                                <input type="hidden" name="user_id" id="user_id" value="{{ old('user_id',$user->id) }}" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-1.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="id" required="">
                                    <label for="name" class="text-xl font-large text-gray-700 block mb-2 dark:text-gray-300">{{$user->name}}</label>
                                </div>
                                ...
                                <div class="mr-2">
                                    <label for="is_limit" class="text-md font-large text-gray-700 block mb-2 dark:text-gray-300">Limit: {{$user->is_limit}} <br> <strong>{{moneyFormat($user->limitation)}}</strong></label>  
                                </div>                                
